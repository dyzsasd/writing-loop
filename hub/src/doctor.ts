...
// 与 src/scheduler.ts buildSched 的合并方向一致；--cli flag 只影响 run，不进 doctor）。
export function effectiveScheduler(cfg: WlConfig, project: WlProject): { cli: string; promptMode: string } {
...
export function doctorMain(argv = process.argv.slice(2)): number {
  ...
  if (ws) {
    ...
    for (const [key, p] of enabled) {
      ...
      const { cli, promptMode } = effectiveScheduler(ws.config, p);
