...
export function workspacePage(snapshot: WorkspaceSnapshot, base = ""): string {
...
export function newProjectPage(snapshot: WorkspaceSnapshot, base = ""): string {
  const body = `<div class="form-card"><section class="form-intro"><div class="eyebrow">Project onboarding</div><h1>先把一部剧立稳。</h1><p>这里收集的是操作者决定。提交后只生成一份零写入计划；你必须在下一页核对路径、规格、警告和计划指纹，确认后才会一次性发布 Git repo、运行态板、首张大纲票与 config 条目。</p></section>
  <form class="panel form-grid" method="post" action="${esc(at(base, "/projects/plan"))}">
    ...
    <div class="form-section"><h2>原创定位</h2><small>原创时必填；改编时可留空。</small></div>
    <div class="field"><label for="comparables">对标剧与证据</label><textarea id="comparables" name="comparables" maxlength="500"></textarea></div>
    <div class="field"><label for="differentiation">差异化</label><textarea id="differentiation" name="differentiation" maxlength="500"></textarea></div>

    <div class="form-section"><h2>改编预检</h2><small>只有“小说改编”会读取；原著正文不会通过本表上传或复制。</small></div>
    <div class="field wide"><label for="rightsScope">版权授权范围</label><input id="rightsScope" name="rightsScope" maxlength="1000"></div>
    <div class="field"><label for="compressionRatio">主线压缩比</label><input id="compressionRatio" name="compressionRatio" type="number" min="1" max="1000" value="10"></div>
    <div class="field"><label for="highlightCount">S 级名场面数</label><input id="highlightCount" name="highlightCount" type="number" min="0" max="1000" value="3"></div>
    <div class="field"><label for="sourceNamedCharacters">预计具名角色</label><input id="sourceNamedCharacters" name="sourceNamedCharacters" type="number" min="1" max="1000" value="20"></div>
    <label class="field check"><input name="riskAcknowledged" type="checkbox" value="true"><span>若压缩比 &lt;10、名场面 &lt;3 或角色 &gt;20，我已理解风险并仍要求进入计划。</span></label>
...
export function onboardingPlanPage(snapshot: WorkspaceSnapshot, plan: OnboardingPlan, encodedInput: string, base = ""): string {
  ...
  const body = `<div class="form-card"><section class="form-intro"><div class="eyebrow">Preview · no writes</div><h1>确认后才落盘。</h1><p>计划指纹绑定当前 config、模板版本与全部输入。任何一项在预览后变化，提交都会失效并要求重新确认。</p></section>
  ...
  <div class="confirm-box"><b>Plan ID</b><div class="path">${esc(plan.planId)}</div><p>提交会以原子方式预留最终 repo 与运行态目录，在 durable journal 保护下完成 Git 首提交和项目数据；config.json 仍是最后的可见性提交点。目标路径已存在时绝不接管或覆盖。</p>
    <form method="post" action="${esc(at(base, "/projects/create"))}"><input type="hidden" name="payload" value="${esc(encodedInput)}"><input type="hidden" name="planId" value="${esc(plan.planId)}"><div class="toolbar"><button class="btn danger" type="submit">确认并完整立项</button><a class="btn" href="${esc(at(base, "/projects/new"))}">返回修改</a></div></form>
  </div></section></div>`;
