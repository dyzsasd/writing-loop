...
.atlas-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(280px,.38fr);gap:18px}.source-progress{height:9px;background:var(--paper-low);overflow:hidden;margin:8px 0 15px}.source-progress i{display:block;height:100%;background:linear-gradient(90deg,var(--jade),var(--gold))}.chunk-map{display:grid;grid-template-columns:repeat(auto-fill,minmax(88px,1fr));gap:7px}.chunk{min-height:70px;padding:9px;border:1px solid var(--line);background:var(--paper-hi)}.chunk b{font:10px var(--mono)}.chunk span{display:block;margin-top:6px;color:var(--muted);font-size:9px;line-height:1.35}.chunk.surveyed{background:color-mix(in srgb,var(--blue) 8%,var(--paper-hi));border-color:color-mix(in srgb,var(--blue) 55%,var(--line))}.chunk.selected{border-color:var(--gold)}.chunk.done{background:color-mix(in srgb,var(--jade) 10%,var(--paper-hi));border-color:var(--jade)}
...
.visual-summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:1px;background:var(--line);margin-bottom:16px}.visual-summary div{padding:14px;background:var(--paper-hi)}.visual-summary b{display:block;font:600 25px/1 var(--display)}.visual-summary span{display:block;margin-top:6px;color:var(--muted);font:9px var(--mono);letter-spacing:.07em;text-transform:uppercase}.visual-dossier{position:relative;overflow:hidden}.visual-dossier:after{content:attr(data-phase);position:absolute;right:13px;bottom:-4px;color:color-mix(in srgb,var(--ink) 6%,transparent);font:700 34px var(--mono);letter-spacing:-.08em;pointer-events:none}.visual-dossier .visual-facts{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:1px;background:var(--line);margin:14px 0}.visual-facts div{padding:8px;background:var(--paper-low)}.visual-facts b{display:block;font:600 14px var(--display)}.visual-facts span{display:block;color:var(--muted);font:8px var(--mono);text-transform:uppercase}.visual-camera-list{position:relative;display:flex;gap:5px;flex-wrap:wrap;margin-top:10px}.visual-camera-list span{padding:3px 6px;border-left:2px solid var(--blue);background:color-mix(in srgb,var(--blue) 7%,var(--paper-hi));font:9px var(--mono)}
...
function projectCard(project: ProjectSnapshot, index: number, base: string): string {
  ...
  const season = project.currentSeason === null ? "" : project.seasonStrategy === "single-season"
    ? `<span class="chip">单季</span>` : `<span class="chip">第 ${project.currentSeason} 季</span>`;
  return `<a class="project-card reveal${project.enabled ? "" : " paused"}" data-index="${String(index + 1).padStart(2, "0")}" href="${esc(at(base, `/p/${enc(project.key)}`))}">
    <div class="meta"><span class="chip">${esc(format(project))}</span>${project.genre ? `<span class="chip">${esc(project.genre)}</span>` : ""}${season}${live ? `<span class="chip live">创作室运行中</span>` : ""}${!project.enabled ? `<span class="chip warn">已暂停</span>` : ""}</div>
...
function storyCockpit(model: StoryStudioReadModel | undefined): string {
  ...
  const sourceLabel = !source ? "原创" : source.phase === "review-ready" ? "已验收"
    : source.phase === "analyzing" ? `本季 ${source.completed}/${source.selected}`
      : source.phase === "surveyed" ? "全书地图完成" : `全书 ${source.surveyed}/${source.chunkCount}`;
...
export function projectPage(snapshot: WorkspaceSnapshot, project: ProjectSnapshot, notice?: string, extras: ProjectPageExtras = {}, base = ""): string {
  ...
  const seasonLabel = project.currentSeason === null ? "季制未登记" : project.seasonStrategy === "single-season"
    ? "单季完结" : project.seasonStrategy === "multi-season" ? `多季项目 · 第 ${project.currentSeason} 季`
      : `季数待全书扫描决定 · 当前第 ${project.currentSeason} 季`;
  ...
  <section class="project-hero"><div><div class="eyebrow">${esc(format(project))}${project.genre ? ` · ${esc(project.genre)}` : ""}</div><h1>${esc(project.title)}</h1><p class="logline">${esc(project.logline ?? "一句话故事仍待总编剧定稿。")}</p><div class="toolbar"><span class="chip">${esc(seasonLabel)}</span><span class="chip">本季 ${total ?? "?"} 集</span></div><div class="path">${esc(project.repoPath)}</div></div><div class="folio"><strong>${String(project.progress.frontier).padStart(2, "0")}</strong><span>CURRENT EPISODE / ${total ?? "OPEN"}</span><div class="mini-progress"><i style="width:${progressWidth}%"></i></div><span>${pct(project.progress.percent)} COMPLETE</span></div></section>
...
function sourceSection(model: StoryStudioReadModel): string {
  ...
  const surveyPct = Math.round(source.surveyed / source.chunkCount * 100);
  const selectedPct = source.selected ? Math.round(source.completed / source.selected * 100) : 0;
  const chunks = source.chunks.map((row) => `<div class="chunk${row.completed ? " done" : row.selected ? " selected" : row.surveyed ? " surveyed" : ""}"><b>${esc(row.id)}</b><span>${esc(row.headings.slice(0, 2).join(" · ") || "无标题分块")}</span></div>`).join("");
  return `${sectionHeader("Source dossier", "原著分析", "先覆盖全书建立事实地图，再为当前季选择深度证据；这里不展示或复制原著正文。", `${source.surveyed}/${source.chunkCount}`, "WHOLE-BOOK SURVEY")}
  <div class="atlas-grid"><section class="panel"><div class="panel-head"><h2>${esc(source.title)}</h2><p>${source.chunkCount} 个不可变分块</p><span class="aside">${esc(source.phase)}</span></div><small>全书结构扫描 ${source.surveyed}/${source.chunkCount}</small><div class="source-progress"><i style="width:${surveyPct}%"></i></div><small>当前季深度证据 ${source.completed}/${source.selected}</small><div class="source-progress"><i style="width:${selectedPct}%"></i></div><div class="toolbar"><span class="chip">SHA ${esc(source.sha256.slice(0, 16))}…</span><span class="chip">${esc(source.allowedHarnesses.join(" / "))}</span><span class="chip">${Math.round(source.byteLength / 1024 / 1024 * 10) / 10} MiB</span></div><div class="chunk-map">${chunks}</div></section><aside class="stack"><section class="panel"><h2>证据边界</h2><p>原著正文保留在 workspace 的 0600 运行态；Git 只接收摘要、来源指纹与人工核验结果。</p><div class="path">${esc(source.planId)}</div></section><section class="panel"><h2>状态语义</h2><p><span class="chip">白色</span> 尚未扫描</p><p><span class="chip">蓝色</span> 已纳入全书地图</p><p><span class="chip warn">金色</span> 当前季已选、待深拆</p><p><span class="chip live">绿色</span> 当前季已 checkpoint</p></section></aside></div>`;
}
...
export function renderVisualProductionSection(model: StoryStudioReadModel): string {
  const scenes = model.story?.assets.scenes ?? [];
  const visual = model.visualProduction?.manifest ?? null;
  const productionByScene = new Map((visual?.scenes ?? []).map((row) => [row.sceneId, row] as const));
  const phaseLabel: Record<string, string> = {
    planned: "布景计划", blockout: "3D 灰模", "passes-ready": "约束图就绪",
    "keyframe-review": "关键帧审看", approved: "关键帧批准",
  };
  const initialized = productionByScene.size;
  const blendSets = [...productionByScene.values()].filter((row) => row.blendAsset !== null).length;
  const renderPasses = [...productionByScene.values()].reduce((sum, row) => sum + row.renders.length, 0);
  const approved = [...productionByScene.values()].reduce((sum, row) => sum
    + row.candidates.filter((candidate) => candidate.status === "approved").length, 0);
  const summary = `<div class="visual-summary"><div><b>${esc(initialized)}/${esc(scenes.length)}</b><span>已进入视觉生产</span></div><div><b>${esc(blendSets)}</b><span>Blender 布景</span></div><div><b>${esc(renderPasses)}</b><span>空间约束通道</span></div><div><b>${esc(approved)}</b><span>已批准关键帧</span></div></div>`;
  const issue = model.visualProductionError
    ? `<div class="toolbar"><span class="chip warn">视觉生产清单不可用：${esc(model.visualProductionError)}</span></div>` : "";
  const cards = scenes.map((row) => {
    const production = productionByScene.get(row.id);
    const phase = production ? phaseLabel[production.phase] ?? production.phase : "尚未排入";
    const candidateCount = production?.candidates.length ?? 0;
    const approvedCount = production?.candidates.filter((candidate) => candidate.status === "approved").length ?? 0;
    const cameraList = production?.cameras.map((camera) => `<span>${esc(camera.id)} · ${esc(camera.label)} · ${esc(camera.lensMm)}mm</span>`).join("") ?? "";
    return `<article class="dossier visual-dossier${row.primary ? " primary" : ""}" data-phase="${esc(production?.phase ?? "waiting")}"><span class="tier">${esc(phase)}</span><h3>${esc(row.name)}</h3><small>${esc(row.id)}${row.variantOf ? ` · 复用 ${esc(row.variantOf)} 几何` : " · BASE SET"}</small><p><b>复用策略：</b>${esc(row.reusePlan ?? "尚未建立；若只用一次会被 S05 拦截")}</p><div class="visual-facts"><div><b>${esc(production?.cameras.length ?? 0)}</b><span>机位</span></div><div><b>${esc(production?.renders.length ?? 0)}</b><span>约束图</span></div><div><b>${esc(approvedCount)}/${esc(candidateCount)}</b><span>批准 / 候选</span></div></div>${cameraList ? `<div class="visual-camera-list">${cameraList}</div>` : `<small>等待 Blender 灰模、固定机位与灯光状态。</small>`}<div class="toolbar"><span class="chip">${production?.blendAsset ? `几何 r${esc(production.geometryRevision)}` : "未登记 .blend"}</span><span class="chip">${production?.lightingStates.length ?? 0} 灯光状态</span><span class="chip${approvedCount ? " live" : ""}">${approvedCount ? "H3 输入可选" : "H3 未启动"}</span></div></article>`;
  }).join("");
  return `${sectionHeader("Virtual set ledger", "美术资产", "故事结构只定义场景；这里登记 Blender 布景、固定机位、Depth/Normal/Lineart 等约束图与人工批准关键帧。二进制只保存为 AssetRef，H3 仍是后续镜头层。", String(scenes.length), "BLENDER → KEYFRAME")}
  ${issue}<section class="panel">${summary}<div class="art-grid">${cards || `<div class="empty">故事结构尚未派生场景资产。</div>`}</div></section>`;
}
...
export function projectStoryPage(snapshot: WorkspaceSnapshot, project: ProjectSnapshot, section: StoryStudioSection,
  ...
  const content = section === "source" ? sourceSection(model) : section === "story" ? storySection(model)
    : section === "timeline" ? timelineSection(model) : section === "assets" ? assetGraphSection(model)
      ...
      : section === "art" ? renderVisualProductionSection(model) : qualitySection(model);
...
export function newProjectPage(snapshot: WorkspaceSnapshot, base = ""): string {
  ...
  <section class="panel onboarding-contract"><article><div class="eyebrow">YOU PROVIDE</div><h2>原著 + 总体改编方向</h2><p>剧名、受众、季制、本季集数、本地原著路径、权利边界，以及你希望保留、改写或避免的原则。</p></article><article><div class="eyebrow">WRITING-LOOP DECIDES</div><h2>任务计划 + 创作产物</h2><p>先扫描整部原著，再提出季界、当前季取材、人物与场景资产、分集节拍、质量门和 tickets，全部由编剧室自主生成并接受 Showrunner 审核。</p></article></section>
  <form class="panel form-grid" method="post" action="${esc(at(base, "/projects/plan"))}">
    ...
    <div class="field"><label for="seasonStrategy">季制</label><select id="seasonStrategy" name="seasonStrategy">${option("single-season", "单季完结")}${option("multi-season", "多季项目")}${option("undecided", "全书扫描后决定", true)}</select><small>决定 Source Analyst 如何提出季界；不会限制其先读完整部原著。</small></div>
    <div class="field"><label for="currentSeason">当前开发季</label><input id="currentSeason" name="currentSeason" type="number" min="1" max="100" value="1" required></div>
    <div class="field"><label for="totalEpisodes">当前季集数</label><input id="totalEpisodes" name="totalEpisodes" type="number" min="1" max="300" value="80" required></div>
