...
writing-loop production plan-shots --plan --project demo --input batch.json --config production-runtime.json \
  --from-script 1 --scene 1 --shot EP001-S1-1   # --from-script takes the episode number; --shot filters shots
writing-loop production plan-shots --confirm <batchPlanId> --project demo --input batch.json --config production-runtime.json
writing-loop production evidence register --project demo --kind license --file LICENSE \
  --config production-runtime.json              # store one evidence file in the workspace CAS
...
| `source status --project K [--json]` | inspect whole-book surveyed chunks, current-season selected/completed evidence, and phase |
| `source survey-start\|survey-checkpoint\|survey-finalize …` | Source Analyst's resumable whole-book survey gates; scheduler-managed in normal onboarding |
...
| `production plan-shots --plan --project K --input FILE --config RUNTIME [--from-script <episode> [--scene N]…] [--shot ID]… [--json]` | compile a shot batch against the gateway-exported profile snapshot and return a zero-write approval document: per-shot estimate, backend rationale, carry-chain waves (a `previous-shot-last-frame` keyframe must name an upstream take that is already in the ledger at `qc-pending`/`approved` with a byte-identical last frame, so a batch never chains within itself and always has exactly one wave), degradations, validation summary and a `batchPlanId`. `--from-script` takes the episode number (`--from-script 1 --scene 1`); `--shot` selects individual shots (repeatable, intersected with the batch document's `shotIds`) and unselected shots are listed as `selected: false` without being compiled, enqueued or estimated |
| `production plan-shots --confirm BATCH_PLAN_ID --project K --input FILE --config RUNTIME [--shot ID]… [--json]` | publish that exact batch: per shot write the immutable ShotRequest into the workspace CAS and a readable batch-approval record (`batchPlanId`, `samplePolicy`, `taskIdPrefix`) next to the immutable intent, then enqueue with the shot's own single-intent planId; `phase: bulk` first requires every named sample task to be approved |
...
| `production evidence register --project K --kind rights\|moderation\|license --file PATH --config RUNTIME [--status S --reviewed-at ISO] [--json]` | store one rights/moderation/license evidence file in the workspace CAS (content-addressed, so re-registering is a no-op), sniff its media type from the bytes, and print the AssetRef plus the object fragment to paste into the batch document; `--kind moderation` additionally requires `--status` and `--reviewed-at`, because the verdict and review time are operator facts the file itself cannot evidence |
