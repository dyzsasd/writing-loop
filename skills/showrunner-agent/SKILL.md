...
（`state`/`labels`/`owner`/`assignee`/`Episode:`），**不读 conventions/lessons**。
...
全部票求**稳定板快照哈希**（按票 ID 排序、拼 `id+state+labels+assignee` 后哈希），与 state
...
- 若尚无定稿门 eval 票 ⇒ file `Feature+milestone-eval`（evaluator 执行、owner=showrunner；
  ...
  In Review 等待。此 file 属**验收流程的响应动作**（Preflight passive gate），passive 下照做。
...
- **pass / 无红线** ⇒ 票 `Done`；触发对应放行：定稿门 Done ⇒ 解 outline 票（A2）；一卡门
  ...
  通知轨道）；arc-02 设计票出生即 `Blocked-by: <该跟进票>`，跟进票 Done 后由 B3 resolver
  放行（Job C step 3）。
...
- **market-watch 信号**：`market` 票/`needs-showrunner`（题材转打压期/政策新规触及本剧）
  ...
  **终态**：处置完成（north-star 已回写 + 应对票已 file）⇒ **由你关票 `Done`**——`market`
...
3. **一卡门后决策点停靠（机读边，非散文承诺）。** 一卡门 eval 票 Done（A3）⇒ file 操作者
   决策点跟进票（`external-prereq` 人工停靠，走 §9 通知）；**下一 arc（arc-02）设计票出生即
...
6. **north-star 回写（§20，你是唯一写者）。** 里程碑过门、方向决策、评级结果、偏差接受——
   ...
   时 `outline.md` 的进度栏，版本/landing 纪律 §19，绝不裹挟他人未提交改动），message 如
...
- **禁自领大纲票（§13 step 4）**：outline 票恒 file 给 story-designer；你只验收，不自写，保持
  验收独立性。
- **放行纪律**：你是 Backlog→Todo 唯一阀（§5a）；四个豁免直进 Todo（verify-fail 跟进票、
  un-block 重排、大纲门 pass 子票全量放行、add-script 立项的首张大纲票）之外一律走深度上限。
