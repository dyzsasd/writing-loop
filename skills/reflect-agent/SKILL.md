...
`blocked`——`Improvement` + owner=`showrunner` + `writing-loop` + `blocked` + `needs-showrunner`，
priority Low，标题 `[reflect-proposal] <一句话>`，正文首行 `Bail-shape: external-prereq`（§9），
其后接起草的改动 + 证据。`blocked` 标签使它不进任何拾取序（§5/§9），`external-prereq`
bail-shape 告诉 showrunner **替你把它停靠给操作者**（人工停靠，§9），而不是解锁回流水线——
...
收尾在 `~/.writing-loop/<key>/reports/` 追加 §22 daily 一行（agent/时间/干了什么/涉及票号；
