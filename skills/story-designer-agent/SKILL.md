...
- **采纳** ⇒ 改节拍单走 **§19 delta 复审工序**（大纲门后改 arc 的强制程序）：①在
  ...
  `continuity` 复核票；④改动区局部重验交 showrunner（R1/R2）。自主 commit 改后节拍单。
- **不采纳** ⇒ 在提案所在票评论说明理由（引本集节拍/账本事实），移除 `needs-designer`
  标签（防重复处理）。

**B. arc 账本滚存核对（§19 rollup）**。若某已完成 arc（其全部 `episode` 创作/重写票
Done）的明细尚未归档，把 `ledgers/story-state.md` 该 arc 的逐集末态摘要 + `foreshadow.md`
该 arc 已 `paid` 的条目滚存到 `ledgers/archive/arc-NN.md`（留一行滚存索引），活跃账本
只保留「当前值 + 本 arc 窗口」（≤15KB 纪律）。账本写操作走锁协议（§15.5/§18：写前
O_EXCL 创建 `<file>.lock`，>60min 陈旧强清）。此滚存的自然时机是在下一 arc 设计票内
（story-state 模板已如此约定）——若你本 fire 正拾 arc-(k+1) 设计票，在其 Step 4 内一并做。

## 2. 主循环（重复到每 fire 上限）

### Step 0 — 回收孤儿（崩溃恢复，§7）
查 项目 + `writing-loop` + `In Progress` + `story-designer` tier。逐张按其**模式**判定：
...
2. **候选竞争与弃案**（大纲门检查项——单案直提 = 平庸风险）：反转拍/危机拍/尾钩各留
   **≥2-3 组备选拍案 + 弃案理由**（如「弃：与 arc-03『录像底牌』同构」）。
