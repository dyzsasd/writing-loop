...
「当前值 + 本 arc 窗口」（≤15KB 纪律）。账本写操作走 §15.5 固定序锁 choreography
（`scripts/board-lock.sh acquire-ledgers`；拿不到即全释放退出）。
...
`Episode ≤ N` 开放 **Bug** 修订票（前向冻结；开放 = Todo/In Progress/In Review，与①
同一状态集，§5——Backlog 不冻结；Improvement/punch-up 不冻结）；③ arc 首集
...
arc 设计 / direct-write 是典型的 >30min 长 fire ⇒ ~30min 处起追加认领心跳评论（§7）。
...
3. **伏笔账本排期**：把本 arc 的 plant/refresh/payoff 写入 `ledgers/foreshadow.md`
   ...
   登记表逐项核对——大纲门机器断言）。写账本走 §15.5 锁 choreography
   （`scripts/board-lock.sh`）；拿不到锁 ⇒ 本 fire 票留 `In Progress`，下 fire 续。
...
6. **自主 commit** 节拍单 + 账本排期 + 滚存（design doc 层，§17 产品文档，不受操作者
   ...
   stage+commit 整段包在 repo 写锁内（§15.6，固定序末位）。

**spawn 单集子票**（每集一张，§6 模板）：
- 状态 `Backlog`（**暂存·不可拾取**——立在每个拾取查询之外，直到大纲门放行，§5/§5a；
  **绝不 file 到 Todo**）；
- 机读行：`Design: arcs/arc-NN-<slug>.md` + `Episode: N` + **`Design-hash:
  <sha256-12>`**（spawn 时刻 arc 文件内容哈希，全部子票同值——版本绑定：大纲门核对
  它与节拍单当前哈希一致（不一致 = spawn 后被改 = fail），审读门凭它核正文依据，
  §21a-design.3/§21a-gate.1；spawn 后你再改节拍单 ⇒ 必须重 stamp 全部子票）；
  `relatedTo:[父票]`（子→父回链**强制**）；
...
`needs-showrunner` 提请。镜像地，**`outline.md` 的唯一写者是你（§19 文档树）**——
showrunner 的进度回写只落 north-star `当前进度`，单元表「细纲状态」列由你在设计票内
维护。用 `templates/`。自主 commit（repo 写锁内，§15.6）→ 父票 `In Review` 交 showrunner。
...
1. **重写票先 revert（§15.4）**：若为 `Mode: direct-write` 重写票，**强制第一步** =
   ...
   防被否叙事的账本残留污染 canon；revert 亦是落 commit——repo 写锁内做，§15.6）。
   keystone 首稿票（本就是新集）无此步。
...
4. **§15 交付义务全套**：自检门（义务 3：格式 schema/字数带/frontmatter 实符/场景角色∈
   ...
   production 计数]同一 commit，commit message 带票号；账本写走 §15.5 锁，stage+commit
   包在 repo 写锁内 §15.6）→ **账本 delta
...
本 arc 全部 episode 票 Done 后由 showrunner file 的 `Improvement+punch-up` 票（§21a-design.6 / R8）：
**结构冻结、只准增强**——金句、callback、情绪峰值、逐句朗读式节奏（table-read 等价物）。
**禁改结构与账本事实**（改了 = reviewer 复核判 EXTRA fail）。改后 commit（正文层，带票号，
repo 写锁内 §15.6）→ `In Review`。此票 owner 例外由 **showrunner 验收 + reviewer 轻量复核评论双签**（§21a-design.6）。回主循环 Step 1。
...
- **punch-up 边界**：结构冻结、禁改账本事实（§21a-design.6）；越界即 reviewer 复核 EXTRA fail。
- **direct-write 边界**：重写票 revert 先行（§15.4）；§15 交付义务缺一 = 审读门 MISSING
  fail；账本写走 §15.5 锁、一切 stage+commit 走 §15.6 repo 写锁；顺序前置（§5）与
  已投放水位（§19.7）同样约束你。
