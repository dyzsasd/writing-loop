...
- `∃ state:Todo` + tier=`episode-writer` 的**任意类型**票（Feature/Bug/Improvement 均算——
  修订 Bug/Improvement **无** `episode` 子标签，谓词绝不按子类型收窄，否则 Urgent 修订会被
  cheap-exit 掉）；**或**
- 逃逸口 ② 孤儿：`∃ In Progress` + 本 tier + `assignee` 陈旧（>60min，§7）；**或**
- 逃逸口 ③ 报告结算（§0 铁律）：到期 weekly/monthly 汇总（state 时间戳）或 `reports/` 有
  未分发的 `*.review.md`（一次 glob，§22）。

（逃逸口 ① needs-\* 求助对本角色为**空集**：§4 needs-\* 闭集只有 `needs-showrunner` /
`needs-reviewer` / `needs-designer`——**本角色无 needs-\* 入口**，不存在
`needs-episode-writer`。逃逸口 ④ 仅 showrunner。）
...
单集起草预期超 30min ⇒ ~30min 处起追加认领心跳评论（§7——防 60min 孤儿判据误收割活 fire）。
...
**修订票（Bug）额外义务**：
- **fail-revert（§15.4）义务只属 supersede 跟进票**：仅当本票是 `review failed:` supersede
  跟进票（回炉修订票——原票 Cancel 评论以 `review failed:` 开头且记有失败稿 commit sha；
  direct-write 重写票同理但不归你）时，**第一步 = `git revert` 该失败稿 commit**（正文 +
  账本一体回滚，防被否叙事的账本残留污染 canon；revert 亦是落 commit——repo 写锁内做，
  §15.6），再按 notes 改写。doctor/evaluator 对
  **已 Done 集** file 的修订 Bug **绝不 revert 已验收 canon**——那是通过验收的正式正文，
  按 §19 修订涟漪协议在现有正文与账本上正常修改。
...
1. **账本先更新**：本集产生的伏笔状态（plant/refresh/payoff）、`story-state.md` 当前值 +
   本集末态摘要行 + 被动标记、`production.md` 计数，全部写回 `ledgers/`。**账本锁**：
   走 §15.5 固定序 choreography（`scripts/board-lock.sh acquire-ledgers`）；
   **拿不到锁 ⇒ 本 fire 票留 In Progress、下 fire 续**，不硬写。
2. **单 commit 原子性（§15.1）**：单集正文 + `ledgers/` 全部更新在**同一个 commit**，
   commit message 带票号（landing 恒为 direct-commit 到 main，无 PR）；stage+commit 整段
   包在 repo 写锁内（§15.6，固定序末位——最后拿、最先放）。**工单转态永远在
   commit 之后。**
