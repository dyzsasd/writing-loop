...
1. **机读块实符**（格式门）：script-format §4 机读块（`hook-type` / `words` /
   ...
   指纹（`beat-card-hash`/`model`/`rules-version`）缺失 ⇒ MISSING。**版本绑定
   （§21a-gate.1）**：`beat-card-hash` 必须 == 票上 `Design-hash` 机读行（大纲门批准
   的版本），**或**经 arc 文件头 changelog 的 `prev→new` 哈希链自 `Design-hash` 可
   逐条追到 `beat-card-hash`（每条 delta 复审条目记哈希对，§21a-design.5）——断链 =
   fail（正文写在未过门的节拍单版本上）。缺 `Design-hash` 行的存量票 ⇒ 本项退化为
   指纹齐全判据，评论注明无版本锚（doctor 哈希比对兜底）。
...
- **邻集复核票（完成时同一动作 file，§6）**：受影响 ⊆ ep-N±1 且修订 pass ⇒ **在同一
  ...
  `tier:episode-writer`，`Episode:` 带邻集号，**`relatedTo:[本修订票]`（回链强制，
  §6——链是跳数的载体）**，AC =「ep-N±1 与修订后 ep-N 的承接帧/钩子
  兑现/信息位阶一致；不一致处已修复」，**直进 Todo**（verify-fail carve-out 语义，
  §5a/§6）。**递归上限 ≤2 跳（§19 step4）——跳数的机械求值 = `relatedTo` 链回走**
  （与 §21a-fail supersede 链同一标准：位置从票上求值，绝不靠记忆）：你正要 file 的
  复核票 `relatedTo` 本修订票；本修订票若又 `relatedTo` 某张更早的邻集复核票，继续
  往回走——链上已有的「复核→再修订」环数 ≥2 ⇒ 你**不再自动开票**，转人工停靠
  （§9 载体：`blocked`+`needs-showrunner`——修订风暴该由人重新决策）。
...
3. **人工停靠**：任何 `Mode: direct-write` 票**再 fail** ⇒ Cancel + file 停靠票
   `Bail-shape: fix-exhausted`（§9 人工停靠载体：`blocked`+**`needs-showrunner`**+
   该 Bail-shape 行——不带 `needs-showrunner` 的停靠票对一切扫描隐形；走 comms 通知
   / needs-attention 轨道与 24h 重提醒，首条 `Notified: <时间戳>` 评论由 showrunner
   B1 记），不再回炉、不再自升级。
