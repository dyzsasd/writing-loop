...
description: >-
  ...
  episode tickets against the seven-item 审读门 checklist (every narrative
...
创作档 opus/max）⇒ 记一行日志、该集验收留 `In Review` 待更高档 fire，**不强验**
（低档验高档产物违反独立性纪律的初衷）。
...
「Feature⇒showrunner」改回）、**全部 `Bug`**（修订票）、**reviewer 所 file 的
Improvement**。查询：项目 + `writing-loop` + `owner:reviewer` + `state:"In Review"`，
按 created 升序（FIFO）。逐张认领（评论 + token）后按票类分三支处理。

#### A-1. 单集创作票（episode / direct-write 重写票）—— §21a 审读门七项清单

对照 `Design:` 节拍单逐项过 §21a **审读门七项清单**。铁律：**每条叙事断言必附正文
...
文件缺、档位低于创作档）⇒ **不转 Done**，留 `In Review`，评论一行原因，下 fire 复验
（先尝试补证再按 fail 处理）。**任一门命中 ⇒ fail，走 A-4 三级路由。**
...
1. **默认 = notes 回炉**：Cancel 原票（评论 `review failed: <败因分类>; superseded by
   ...
   **至多 2 轮**：轮次 = supersede 链长度（机器可数——沿 `relatedTo`/`superseded by`
   链回溯本 Episode 被 Cancel 的次数）。
...
- **inconclusive 永不算 pass（§3）**：引不出正文、账本锁不到、文件缺、本 fire 档位低于
  创作档——任一情形都留 `In Review` 复验，绝不给未取证的集判 Done。判决必须有观测证据
  （正文引文 / 账本行），否则只是意见。
