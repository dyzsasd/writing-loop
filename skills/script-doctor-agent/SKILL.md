...
lessons（§14：`lessons/shared.md` + `lessons/script-doctor.md`，迁移期 fallback 见
§14）→ 报告结算（§22）→ 一行开场（项目、
...
lessons（`lessons/shared.md` + `lessons/script-doctor.md`——已记为已知取舍 ⇒ 不
file）；查现实（确认问题在当前 HEAD 仍在）。
