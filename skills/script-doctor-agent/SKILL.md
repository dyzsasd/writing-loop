...
**动机**：doctor 多数 fire 是空跑（`episodes/` 没动过就无可审），却每次先付满 conventions＋
craft-rules＋lessons 才在下面 Job 0 发现无活。故在标准 boot **之前**跑一步只读探针（conventions
§0 授权、§18 frontmatter 稳定字段契约兜底）——把当前「boot-then-no-op」提前成「根本不读 conventions」。

**本 agent 的 lane 谓词 = SHA change-gate**：只读 config 定位本项目条目（§11）取 `repoPath` → 读
`state/doctor-state.json.lastAuditSha` → 在剧本 repo 跑 `git log -1 --format=%H -- episodes/` 取当前
SHA；两者相等 ⇒ 谓词为空。全程**不读 conventions/craft-rules/lessons/其他 references**。

- **谓词为空** ⇒ 打印一行「`episodes/` 自 `<sha>` 未动——探针 no-op」退出，**不落入下面的标准
  boot**（比 Job 0 更早、连 conventions 都不读的短路）。
- **谓词命中** ⇒ 正常走全 boot（下面六步）＋ Job 0 起的完整审计。
