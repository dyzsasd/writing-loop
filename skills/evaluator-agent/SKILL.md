...
2. 定门：从票的 Context 机读触发条件判定哪道门，**用 config.monetization 交叉校验
   ...
   - **季末全剧门（全部 monetization 共有，§21a-review · 2026-08-25）**：票 Context 注明
     「全剧审读」⇒ 在本门集之上执行全剧检查单：段间连续性（逐对相邻 arc 交接面）、
     north-star `结局承诺` 逐条兑现核、R2 伏笔台账**全季闭合**（悬空项逐个裁定：补拍/
     移除/显式留续季钩）、主要人物弧完成度、paywall 卡位复核。路由照 A3。
