...
- **title**：剧名（人可读）。
- **repoPath（剧本 repo 目录）**：默认 = **当前工作目录**（GUIDE 约定操作者先 `cd` 进
  剧本目录再跑本 skill）——把该默认值显式呈给操作者**确认或改写**，绝不静默采用。若 boot
  判定为本 workspace **首剧**，workspace 根随之确立，问答须完整走一遍：默认取 repoPath 的
  **父目录**为 workspace 根（剧本 repo 作为其子目录，config 里 `repoPath` 记相对目录名，
  §11）——同样请操作者**确认或改写**，确认后才在其下创建 `.writing-loop/`。
...
- **可选**：`assetLibrary`、`marketDataPath`、`comms.{provider,webhookEnv}`、
  `intake.{mode,todoDepthCap}`、`models` / `efforts` 覆盖、`mode`（live|dry-run）。
  `intake.mode` 默认 `autonomous`；`passive` = 纯用户驱动创作（§5a）——立项时明确询问。
  （v1 恒两层创作、keystone 集恒由 §21a 硬规则自动判定——两者不是配置项，不采集。）
...
- **原著文本入 `source/`**。
- **选书检查表评估**（craft-rules R11）——可改编性三阈值**定死**：**主线可压缩比 ≥10:1**
  （几十万字 → 约 5 万字剧本）、**S 级名场面 ≥3 处**、**具名角色可压至 ≤20**。任一不满足
  ⇒ **明示风险**（指出是哪项、差多少、意味着什么）并要求操作者**显式确认**后才继续，
  绝不静默放行。
...
2. **实例化文档树**（§19 文档树）：
   ...
   - **`README.md`（repo 根，文档索引——操作者浏览剧本 repo 的第一入口，KB 可发现性）**：
     把 §19 文档树静态渲染成一页表，每路径一行「是什么 / 谁写 / 谁读 / 怎么改（经哪道
     门：§19 delta 复审 / §20 节分级 / §21a 大纲门 / §15 交付义务）/ 归档在哪」；另加
     `evaluation/` 报告命名约定（片名+里程碑名，superseded-by 标记语义）与 workspace
     状态目录指针——**点名** `<workspace>/.writing-loop/<key>/state/market-assessment.md`
     的完整路径（评估报告市场层引用它；只拷 repo 不拷 workspace 会断证据链，§11）。
     内容从 INTERVIEW 结论一次生成，描述文档**类**不描述实例（实例已有 outline 单元表
     / 账本滚存索引登记）——showrunner 不维护它，它不属任何门禁。
3. **首个 commit**：`git add -A && git commit`（message 如
   `chore(scaffold): 立项 <key> — bible/outline/ledgers 空骨架`）。staging 纪律 §15.1——
   只提交本 skill 生成的文件，绝不裹挟无关改动。
...
- **ID 分配**（§18，O_EXCL 竞争安全）：读 counter 取起始 N → **独占创建**
  ...
  内容、尽力回写 counter。state 存 frontmatter `state: Todo`（大纲票直进 Todo——依据
  **§5a 第四豁免：add-script 立项时 file 的首张大纲票**，workspace 的第一张票，此时尚无
  Backlog 可梳理，等 showrunner 放行只会空转一个周期）。追加建票时间戳评论（§18）。
