...
description: >-
  ...
  project entry into `~/.writing-loop/config.json` under the config-schema validation
...
2. 定位 workspace 索引 `~/.writing-loop/config.json`（`WRITINGLOOP_DATA_DIR` 可覆盖）——
   **反转**：不是定位既有项目条目，而是**确认目标 key 尚不存在**（key 全 workspace 唯一，
   config-schema 校验）；索引文件不存在 ⇒ 这是本 workspace 首剧，稍后创建索引。
...
> （`~/.writing-loop/<key>/` + 索引 `config.json` 内本剧条目）。绝不触碰他剧的 repo / 板 /
...
1. **config.json 项目条目**（`~/.writing-loop/config.json`，schema 见 config-schema）：写入
   `title` / `repoPath` / `backend:"local"` / `ticketPrefix`（默认 `WL`）/ `mode` / `enabled`
...
2. **config-schema 校验规则逐条**（写入前**必须全过**，不过 ⇒ 回问操作者修正，绝不写入非法
   config）：
   - `repoPath` 存在且是 git repo（SCAFFOLD 已保证）；board 目录专用（空或 writing-loop 脚手架）。
...
3. **创建板目录**（§18）：`~/.writing-loop/<key>/board/` +
   `board/counter.json`（`{ "prefix": "<ticketPrefix>", "next": 1 }`——起始提示，非真相源）+
   空 `board/tickets/`。板目录必须**专用**（空或本系统脚手架），绝不共享 / 网络盘 / commit。
4. **数据目录其余**（config-schema 数据目录布局）：`~/.writing-loop/<key>/reports/`、`state/`。
5. **lessons.md 分节脚手架**（§14）：`~/.writing-loop/<key>/lessons.md`，建全部分节标题（空）：
...
2. **板可写 / 可读**：glob `~/.writing-loop/<key>/board/tickets/*.md`（**严格限定本项目板
...
- **§2 安全边界**：写操作只落**新剧本 repo** + **本项目数据目录**（`~/.writing-loop/<key>/`
...
立项成功收尾，在 `~/.writing-loop/<key>/reports/` 追加 **daily 一行**（agent=add-script /
时间 / 干了什么 / 大纲票 ID）：如
