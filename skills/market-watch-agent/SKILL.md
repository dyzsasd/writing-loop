...
（`~/.writing-loop/config.json`，§11）定位项目条目，读不到 ⇒ 问操作者，**不猜路径** →
...
2. 读 `~/.writing-loop/<key>/state/market-state.json`（你自己的状态文件；不存在则惰性建
...
`~/.writing-loop/<key>/state/market-assessment.md`（你的产物，**不进剧本 repo**）。至少含：
...
在 `~/.writing-loop/<key>/reports/` 追加 **§22 daily 一行**（agent / 时间 / 干了什么 /
