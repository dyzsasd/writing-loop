...
- **④ doc-watch（仅 showrunner）**：操作者改 `north-star` 时可能尚无板票 ⇒ showrunner **永不**
  纯退出，降级为「cheap boot」（省 conventions/lessons，但仍读 north-star 算哈希）。
...
> 且当前无任何东西检测它。**sweep 每 fire 检测**：`∃` 带 `keystone` 标签的 In Review 票，
> 其 `updated` 陈旧（> 阈值 T，如 30min）且近窗无顶配 reviewer 迹象 ⇒ 在板健康 digest **旗标**
> （`keystone 集 <ID> 停滞，需顶配 reviewer`），把静默 stall 浮出给操作者。有 launcher 时，
> launcher 直接**按被门控票的 `keystone` 标签计算所需 reviewer 档位**（读票 tier→定 effort
> 起 fire），使「跳过留待」分支基本永不触发——但最终 floor 判定仍由 reviewer agent 自己按本
> 规则做（launcher 只 advisory 选档，§0/§18 单一真相源不变）。
