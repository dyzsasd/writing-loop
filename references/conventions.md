...
探针是**本地纯板判定**，与外层 work-gated dispatch（内建调度器 scripts/wl-run.py）**正交互补**：dispatch
...
> 旗标的操作者判断。内建调度器 `scripts/wl-run.py` 起 reviewer fire 前 glob 板
...
`scripts/wl-run.py` 驱动时该前提是**构造保证**——写 repo 四角色全局单飞、板上角色不
...
wl-run 进程自己的 UTC 时钟逐 fire 记账）为可信时间源求值——机制细节见 config-schema
