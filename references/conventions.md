...
6. **repo 写锁（`repo.lock`）**：任何向剧本 repo 落 commit 的 stage+commit 序列——
   ...
   `.git/` 下使锁文件永不可能被 stage 进 commit）。锁内只做 stage+commit 本身，慢活
...
stage+commit 包进 repo.lock 已足——**不必 worktree**，别为单发配置徒增机器。操作者
...
- **doc-watch（自触发排除——异源版本规则）**：每 fire 对比 state 目录快照哈希。
  ...
  刷新绝不先于写完）。**回写前必重验（中途竞态守卫）**：每个回写点在 repo 写锁内、
...
3. **spawn 单集子票**：每集一张（§6 模板），`state:"Backlog"` 暂存、
   ...
   机读行、`relatedTo:[父票]`；keystone 集（**前 3 集**、
...
- **审计侧**：活跃账本 ≤15KB 纪律（§19 文档树）的稽核方 = **sweep digest**（超编
