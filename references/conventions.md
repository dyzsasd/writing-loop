...
Workspace 索引 `~/.writing-loop/config.json`（schema 全文见 `config-schema.md`）。
每项目关键字段：`repoPath`（剧本 repo）、`format`（live-action|ai-anime|reelshort-en）、
...
3. **注册**：写 `~/.writing-loop/config.json` 项目条目（校验规则见 config-schema）；
...
`~/.writing-loop/<key>/lessons.md`，分节：
...
4. **fail-revert 协议**：reviewer 判 fail 时在 Cancel 评论记录失败稿 commit sha；
   ...
   一体回滚，防被否叙事的账本残留污染 canon）。sweep 稽核「Canceled 单集票且其
   commit 未被 revert」。
...
板目录：`~/.writing-loop/<project-key>/board/`（`WRITINGLOOP_DATA_DIR` 可覆盖）。
