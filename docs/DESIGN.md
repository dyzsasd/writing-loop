...
**空跑 fast-path（首个项目实测后加，§0 Step-0）**：首个项目实测 121 fire / 107 no-op
（88%）——写作工作**极度阶段性 + 串行单前沿**，任一时刻只有 1-2 个角色有活，其余空跑，
每次仍先读满 ~51KB conventions 才发现无活。对策：boot 前插一步**廉价车道探针**（只读板
frontmatter 求本 lane 谓词，空则不读 conventions/lessons 直接退出），四逃逸口保证单向安全
（宁假命中不假退出）。这是 loop-design 层、本地可独立 ship、回收 ~90% 空跑 token 税。
...
（fire 121→~25-35）。多剧错峰并跑时「9 角色常开」自然回归为对。详见 docs/RESEARCH/wl-opt-*.md。
