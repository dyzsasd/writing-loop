...
| 0-D | `ProductionProviderAdapter` 接口（新文件 `production-provider-adapter.ts`）、`PreparedProviderSubmission` / locator 三 source 联合与严格解析、capability 迁入 `production-adapter.ts`（`limitsByModelId` 以 profileId 为键、`outputRetention`）、`ComfyUiAdapter.capabilities()` 按 profile 集合生成、coordinator 四家族表与 `REQUIRES_SCOPED_STAGING`、errorSummary 词表、runtime-config 家族规则与 `bindings` 判别联合（`h3-graph-bindings` \| `provider-slot-policy`） | §4.3、§4.4、§4.5、§8.6 | 已实现（2026-09-02，2385 PASS），**reviewer 审查中，未提交**。关闭 0-C 遗留 (a)。locator 联合在 gateway/ingestor 侧（1a 禁改）未落，coordinator 对 `provider-output` fail-closed——归 1b | 主工作树 |
...
| 1a | gateway 进程装配（`production-gateway-main.ts`：三内核同进程、bearer、绑私网 IP、优雅停机）、server-owned registry（`production-gateway-runtime-config.ts`，含只读 profile 快照导出）、装配层三客户端接入 `insecure-private-http`、本机 `cas://` resolver、systemd 单元（gateway / worker.service+timer）、`gcp-h3-vm.sh` | §8.2、§8.0 | 已实现并经 reviewer + codex 审查修复（2 blocker + 5 major + 12 minor；2335 PASS），**已提交到分支 `phase1-gateway-assembly`（`e188af0`，已推送），待 0-D 合入后 rebase 合并进 main** | worktree `~/workspace/jinko/wt/writing-loop-phase1` |
