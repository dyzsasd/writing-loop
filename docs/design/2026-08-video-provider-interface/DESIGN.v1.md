...
术语（全文固定）：ShotRequest（不可变镜头请求）、ShotRequestDraft（编译前草稿，下文简称 draft）、intent（`ProductionIntent`，不可变调度意图）、take（handoff 中已批准任务的产物）、候选图（`visual/production.v1.json` 的 `candidates[]`）、execution profile（后端静态参数文件）、Degradation（编译决策记录）、reservation（预算预留）、gateway 实例 A / 实例 B（见 §8.0）。
...
2. 三家后端的统一方式：每个镜头编译为一份内容寻址的 ShotRequest 并作为 intent `inputs[0]`，逐镜变量（模式、时长、seed、输入 slot、prompt）全部在 ShotRequest 内；后端静态参数（provider、modelId、分辨率、画幅、音频开关）在 execution profile 内并构成五项 `workflowBindingKey`；能力差异由 `BackendCapabilities.limitsByModelId` 描述，编译器据此拒绝或降级并写入 Degradation。adapter：Seedance → `ArkVideoAdapter`（operation `ark-video-task`）、Veo 3.1 → `VertexVeoAdapter`（`vertex-veo-lro`）、MiniMax H3 → 现有 `ComfyUiAdapter` 经薄包装（`comfyui-workflow` / `minimax-h3`）。coordinator ↔ gateway 契约与 ProductionTask 状态机不变。
3. 第一条视频的最短路径：Phase 0（契约、编译器、CLI，零网络）→ Phase 1（gateway 实例 A 部署到 writing-loop-sg、TLS 终结、凭据发放、worker systemd timer、Ark adapter）→ 一条 t2v 4 s / 480p / 9:16 任务经 `production plan-shots --plan → --confirm → production-worker --once → production qc --approve → production handoff` 入库（Seedance 2.0 约 0.28 USD）；BytePlus 账户未就绪时 Phase 1 改为 Veo fast 4 s / 720p / 9:16（约 0.40 USD）。
4. 需用户决定的事项：(a) Phase 1 后端裁定（BytePlus 账户、充值 ≥30 USD、密钥是否在 Phase 0 验收前就绪）；(b) TLS 证书方案（私有 CA + `NODE_EXTRA_CA_CERTS`，或内网域名 DNS-01 证书）；(c) GCP 项目 €50/月硬顶：提高硬顶或把视频生产迁到独立项目，`phase: bulk` 前必须决定；(d) 输出语言与口型（`dialogue[].language` 项目级裁定，中文母本与出海英文 profile 并存）；(e) 镜头合并规则（全季约 1910 个 ▲ 行，每集约 60 秒）；(f) MiniMax H3 许可：`useTerritories` / `deploymentTerritories`、`licenseCompliance` 的年收入声明与署名面，含 US/EU/GB/KR 时先申请书面许可；(g) `safety_identifier` 用作对账相关键是否符合平台使用政策（决定 Ark `reconcilePolicy`）；(h) Veo 英文译文的供给步骤（写作侧 agent 或人工）；(i) runtime `projects[].availableBudgetMicros` 的首次取值与维护责任。
...
| 部署 | writing-loop-sg（10.148.0.5）；GCP 镜像 `wl-comfy-h3-g4-sg`；GCP 项目 jinko-vibe-coding | 服务器 workspace 为账本，本机 `~/dramas` 为 paused 副本；镜像未跑过推理；项目 €50/月硬顶（`stop_billing`）覆盖 VM、调度器与 Veo；服务器 ffmpeg / Node / systemd 权限未核实 |

## 3. 三家后端能力矩阵

| 能力项 | Seedance（火山方舟 / BytePlus ModelArk） | Veo 3.1（Vertex AI） | MiniMax H3（ComfyUI，自托管） |
|---|---|---|---|
| 接入形态 | 异步任务：`POST /api/v3/contents/generations/tasks` + `GET …/tasks/{id}`；主机 `ark.cn-beijing.volces.com`（CNY）/ `ark.ap-southeast.bytepluses.com`（USD） | 异步 LRO：`:predictLongRunning` + `:fetchPredictOperation`；仅 `us-central1`（`global` 未核实） | ComfyUI `POST /prompt` + `/queue` + `/history` + `/view`；gateway 实例 B 与 ComfyUI 同机 |
...
| 当前就绪状态 | BytePlus 账户、密钥、余额不存在 | 凭据 veo-m2m.json 与 us-central1 已验证 | 镜像已建、未跑过推理；H3 graph 未 live 验证 |
...
writing-loop workspace（control plane，零 provider 网络；固定为 writing-loop-sg 服务器上的 workspace）
  ...
  production-worker --once（systemd timer，间隔 ≤ 6 h）──coordinator──▶ ProductionGatewayAdapter（HTTPS + credential）
  production qc --approve / --reject ──▶ approved / rejected 事件
  production handoff --export-dir ──▶ VideoStudioHandoff v2 + 资产目录 ──rsync──▶ 本机 VCS

私有 gateway（trust domain，server-owned registry；实例 A / 实例 B 见 §8.0）
  jobs kernel ── ProductionProviderAdapter
      ├─ ComfyUiAdapter（现有，H3）
      ├─ ArkVideoAdapter（新，Seedance）
      └─ VertexVeoAdapter（新，Veo）
...
| 凭据、模型 ID、execution profile、价目、并发准入 | gateway registry | `hub/src/production-job-gateway.ts` |
...
非目标：不修改 `ProductionTask` 的 14 个状态与转移表，不新增第三套状态机；不把 VCS 的 `tools/video/*.py` 接入执行路径；不实现分镜自动生成（`camera` 字段由分镜步骤或人工填写）；不建模视频延长（`mode` 保留 `extend` 值但解析拒绝）；worker、gateway、VCS Python 层不调用 LLM（prompt 译文由写作侧 agent 步骤或人工在 draft 中提供）；v1 不提供静态图生成后端（候选图产出为并行人工轨道）。
...
export type Degradation = {
  code: "anchor-mode-selected" | "duration-rounded-trim" | "references-trimmed" | "reference-video-dropped"
      | "reference-audio-dropped" | "seed-not-reproducible" | "negative-prompt-folded" | "native-audio-off-post-dub"
      | "prompt-translated";
...
export type ShotRequest = {
  ...
  provenance: { storyDesignSha256: string; assetsRevision: number; visualProductionSha256: string | null; beatCardHash: string | null; scriptLine: number };
  ...
  output: { aspectRatio: "9:16"|"16:9"|"1:1"|"21:9"; durationSeconds: number; storyboardDurationSeconds: number;
            resolutionClass: "480p"|"720p"|"768p"|"1080p"|"4k"; fps: 24; generateAudio: boolean; seed: number | null };
  continuity: {
...
| `provenance` | `assetsRevision` 为 `story/assets.v1.json` 的 revision；`beatCardHash` 为剧本 frontmatter `beat-card-hash`；`scriptLine` 为 ▲ 行行号 |
...
| `dialogue[]` | `mode` 由对白前缀 `OS` / `VO` 推导（`references/script-format.md`）；`lipSync = mode === "onscreen"` |
| `output` | `durationSeconds` 为编译取整值，`storyboardDurationSeconds` 为分镜原值；`fps` 固定 24 |
...
`ShotRequestDraft` 为 ShotRequest 去掉 `compile`、`anchorMode`、`droppedReferences`、`selectedTranslation` 的形态，允许首尾帧与参考同时存在，另含 `prompt.translations: Array<{ language: string; text: string; negativeText: string | null; authoredBy: string }>`（可为空）。
...
- 时长：`durationSeconds = 后端网格上取整(max(4, storyboardDurationSeconds))`，Veo 取 {4, 6, 8}，记 `duration-rounded-trim`（`requiresReapproval: false`）；Veo 在 `resolutionClass ∈ {1080p, 4k}` 时网格固定为 {8}（保守默认，探针后可放宽）；H3 网格由已配置 profile 集合定义（§5.3）。
- prompt 语言：后端 `limits.promptLanguages` 非 null 且不含 `draft.prompt.language` 时，若 `translations` 中存在受支持语言的译文则选用该译文作为 `prompt.text`，记 `prompt-translated`（`from: 原语言, to: 译文语言`，`requiresReapproval: true`）并写入 `selectedTranslation`；否则返回 `prompt_language_unsupported`。`dialogue[]` 中 `lipSync: true` 且 `language` 不在 `promptLanguages` 内时返回 `dialogue_language_unsupported`（含中文口型对白的镜头不路由到 Veo）。
...
export type ProductionIntentExecution =
  ...
  | ProductionExecutionBase & { operation: "ark-video-task"; modelFamily: "seedance";
      ...
      resolution: "480p"|"720p"|"1080p"|"4k"; aspectRatio: "16:9"|"4:3"|"1:1"|"3:4"|"9:16"|"21:9"|"adaptive";
...
|---|---|---|
| `workflowSha256` | pinned graph 文件 digest | execution profile 文件（`kind: "writing-loop/execution-profile"`，含 provider、modelId、固定参数、allowedModes、durationGrid、limits、priceTable + priceAsOf、requestBuilderVersion）的 canonical digest；沿用 `workflows[].file` 与 `ImmutableWorkflowRegistry` 读取 |
...
- 逐镜约束在编译与 gateway 两处执行：seedance `durationSeconds` 按 modelId 分档（2.0 系列 4–15，2.5 为 4–30），2.5 在 i2v / fl2v 模式 `aspectRatio` 必须 adaptive，`seed` 必须 null；veo ref2v 固定 8 s，`reference_image` ≤3（或 1 张 style）、`reference_video` / `reference_audio` 为 0；veo `seed` 为 uint32 但仅 best-effort，Veo 分支编译时总是记录 `seed-not-reproducible`（`requiresReapproval: false`）且 `fingerprint.seedReproducible = false`；veo `resolution ∈ {1080p, 4k}` ⇒ `durationSeconds = 8`（保守默认，来源为 Gemini API 文档与 `tools/video/veo_video.py`；Phase 3 探针写入 `profile.limits` 后可放宽）。
...
| `tariff` | provider 不报告实际费用时，按 profile 价目 × ffprobe 实测秒数。Veo 与配置了 GPU 费率的 H3 采用；未配置费率的 H3 保持 `unknown/provider-not-reported`。tariff 不覆盖 GPU VM 的开机、权重加载与空闲小时，这部分进入 §4.7 的月度余量门 |
...
| 层 | 机制 | 增量 |
|---|---|---|
| 批次人工审批 | `production plan-shots --plan` 输出 `ShotBatchPlan`；`--confirm <batchPlanId>` 才写入，内部对每个 intent 以其自身 planId 调用 `commitProductionTaskEnqueue`（`hub/src/production-enqueue.ts` 的确认指纹为单 intent planId），批次 planId 只用于批次审批 | `batchPlanId = sha256(canonicalJson({workspace, project, intents[], policyDigest, degradations[]}))`；plan 文档含每镜估算（写明 modelId 与价目档）、总估算、后端选择理由 `decisions[]`、`phase: sample \| bulk`、`waves[]`（承接链顺序）、`droppedReferences`、`validation` 汇总、GCP 月度余量占用；策略变更即 batchPlanId 失效 |
...
| 估算 | `budget.estimatedAmountMicros` 由 profile 价目 × durationSeconds（Seedance 按 token 公式，含二维价目与最低 token 用量）得出；`maximumAmountMicros = ceil(estimated × 1.5)` | `plan-shots --config <runtime config>` 经 `ImmutableWorkflowRegistry` 读取 profile 文件（零网络），价目单一来源 |
...
| 2 | GCP 月度余量与 GPU VM 时间窗批准 | 项目操作者 | runtime `projects[].availableBudgetMicros` 手工维护；H3 批次的 plan 文档附 `vmWindow{start, end, maxHours}`，`--confirm` 时核对 | 月度余量门 |
| 3 | 批次审批 `plan-shots --confirm` | 制片负责人 | writing-loop-sg 服务器 workspace（ssh 执行） | plan 文档：degradations、译文选用、费用、后端理由 |
| 4 | 样片门 | 同上 | `phase: bulk` 的 `--confirm` 检查 sample 均 approved | sample take |
| 5 | QC 裁决 `production qc` | QC 负责人 | 服务器 workspace | take、尾帧、上游状态 |
| 6 | VCS proposal（渲染运行时） | 合成负责人 | 本机 VCS 检查点 | proposal_packet |
| 7 | publish | 发布负责人 | 本机 VCS，含署名检查 | publish_log |
...
GCP 项目级月度余量门。已核实事实：项目 jinko-vibe-coding 存在 €50/月硬顶（预算 `vibe-coding-50eur-hard-cap`，阈值 0.5 / 0.9 / 1.0，Pub/Sub → Cloud Function `budget-cutoff` 执行 `stop_billing`，恢复需手动 `gcloud billing projects link`）；该顶同时覆盖 GPU VM、writing-loop-sg 调度器与 Veo 计费；预算通知非实时；G4 目录价（新加坡）g4-standard-48 按需约 4.09 USD/h、Spot 约 1.55 USD/h（€50 ≈ 54 USD → Spot 约 35 h/月，按需约 13 h/月）；项目现有常驻资源目录价合计已超过 €50/月（抵扣金状态未核实）。EP001 若 20 镜 × 8 s × 0.40 USD ≈ 64 USD，单独即超顶。设计：

1. `plan-shots` 估算把 Veo 费用与 H3 批次的 VM 小时（开机、63 GB 权重加载、空闲；按 Spot 1.55 USD/h × 批准的 `vmWindow.maxHours`）计入 `gcpMonthlyExposureMicros`，与 runtime `projects[].availableBudgetMicros` 比较；该值由操作者按「€50 − 当月固定支出」手工设置并在 config 中记录 `maintainedBy`、`asOf`。
2. GPU VM 启停为独立人工审批点（审批点 2）：每次 H3 批次绑定已批准的时间窗，worker 在窗口外不提交 H3 intent（gate context `vmWindow`）。
3. Veo / H3 批量（`phase: bulk`）前必须先决策其一：提高该项目硬顶；或把视频生产迁到独立 GCP 项目 / 预算，使 `stop_billing` 不影响 writing-loop-sg 调度器。未决策前只允许 sample 阶段。
4. billing 被解除后的恢复流程见 §7。
...
| `execution.aspectRatio` | `ratio` | 2.5 在 i2v / fl2v 模式必须 `adaptive`，否则 `aspect_ratio_unsupported` |
...
| `output.seed` | 不下发 | 2.x 未列入支持，编译强制 `null`；忽略还是报错未核实 |
| `remoteJobId` | `safety_identifier`（remoteJobId 派生的 64 位十六进制串，查询时原样返回） | 用于 `list-window` 对账相关键；政策未确认前 `reconcilePolicy: none` |
...
| `prompt.text` | `instances[0].prompt` | `promptLanguages: ["en"]`：中文母本须选用 draft 译文（记 `prompt-translated`，重审批），无译文 → `prompt_language_unsupported` |
...
| `references[]` 视频 / 音频 | 无字段（上限 0） | 按 `referencePolicy`：strict → `reference_cap_exceeded`；trim → 裁剪并记录（Degradation 词表含 `reference-video-dropped` / `reference-audio-dropped`） |
...
| 模式 i2v / fl2v：`firstFrame` / `lastFrame` | `LoadImage.image` = `providerObjectKey`（模板 sentinel `writing-loop://stage-input/<profileId>/<index>/<slot>`，slot `first_frame` / `last_frame`） | stage kernel 硬链接到 `<root>/objects/<namespace>/<sha256>`，ComfyUI 与 gateway 实例 B 同一文件系统 |
...
| `prompt.text` | v1：`generator.prompt` 在 pinned graph 参数投影内，逐镜 prompt 需逐镜 graph 与 config 条目；v2（Phase 4）：`generator.prompt` 为 sentinel `writing-loop://shot-request/<profileId>/prompt`，materialize 时从 ShotRequest 填入并重新断言整图 | `prompt.negativeText`：契约无对应输入，处理方式未在设计中定义 |
| `output.seed` | v1：pinned graph `RandomNoise.noise_seed`；v2：sentinel `writing-loop://shot-request/<profileId>/seed` | 可复现，`fingerprint.seedReproducible = true` |
...
| ShotRequest（`inputs[0]`） | v1：不进入 inputs；v2：stage 契约在固定绑定前增加 index 0 的 `shot-request` slot（mediaType `application/vnd.writing-loop.shot-request+json`，不绑定 LoadImage，gateway 从 receipt 的 staged object 读取），LoadImage 绑定 index 顺延 | stage kernel 对该类型改为内容校验（解析 JSON、核对 `kind` 与 slot 一致性） |
...
| 主输出 | `RemoteOutputLocator{source: "comfy-view", …}` → ingest 经 `comfyBaseUrl/view`（跨机为 GPU VM 上的 TLS 代理 + bearer） | SaveVideo 在 `/history` 中的分组名以实测写入测试 |
| 尾帧 | 无 | ffmpeg 提取，同 Veo |
| 成本 | 无 usage | `tariff`（配置 GPU 费率）或 `unknown/provider-not-reported`；VM 小时进入月度余量门 |
| 取消 | `POST /queue {delete}` + 门控 `POST /interrupt` | 现有 |
| 许可 | `useTerritories ∪ deploymentTerritories` 命中 EU / GB / KR / US → written-license evidence；`obligations` → gate `license-obligation-unmet` | 现有 gate + 新 gate |
| VM 时间窗 | gate context `vmWindow` | 窗口外不提交 |
| Spot 抢占 | 实例 B 重启后，持久 job record 中重启前 pending / running 且不在 `/history` 的任务 | 改写为 failed:`provider_failed:preempted` |
...
| `output.{aspectRatio, storyboardDurationSeconds, resolutionClass, generateAudio, seed}` | 分镜表（`shots_ep123.json` 镜头 `dur` 为 2–6 s）；项目输出档 9:16 | 分镜 / 人工 |
| `dialogue[].language`、`lipSync` | 项目级裁定（中文母本与出海英文 profile 并存）；`lipSync` 由 `mode` 推导 | 裁定后预填 |
| `prompt.{text, negativeText, language, authoredBy}`、`prompt.translations[]` | 写作侧 agent 步骤或人工 | 写作侧 |
...
| GPU VM 抢占：gateway 实例 B 重启后，持久 job record 中重启前为 pending / running 且不在 `/history` 的任务 | failed | `provider_failed:preempted` |
...
- billing 解除恢复：`stop_billing` 触发后全部 VM 停止，进行中的 H3 任务随 ComfyUI 进程内 history 丢失（AI-SPEC:421-423）。恢复顺序：`gcloud billing projects link` → 重启 writing-loop-sg（调度器、gateway 实例 A、timer）→ 按需重启 GPU VM 与实例 B → worker 对 `submission-unknown` / `orphaned` 对账 → 实例 B 把重启前 pending / running 且不在 `/history` 的任务改写为 `provider_failed:preempted` → 受影响 intent 重新 plan。
- GPU VM 抢占（Spot）：实例 B 的 job record 在持久盘；抢占后手动重启 VM，处理同上一条最后两步。

## 8. 分阶段落地计划

### 8.0 部署拓扑（全部 Phase 的前提）

约束来源：worker 侧 `production-gateway` backend 必须为 HTTPS 且 `credentialEnv` 非空，loopback 不豁免；仅 `gateway`（ingest）与 `stagingProfiles[]` 允许无凭据 loopback HTTP（`hub/src/production-runtime-config.ts`）。gateway 进程以 `node:http` 监听且只绑定字面私网 IP，不含 TLS 终结（`hub/src/production-gateway-router.ts`、`production-gateway.ts`）。minimax-h3 workflow 必须 scoped-staging，scoped-staging 禁止绑定 `kind=comfyui` backend；stage kernel 把资产硬链接到 `<root>/objects/<namespace>/<sha256>` 并把 providerObjectKey 写入 `LoadImage.image`（`hub/src/production-stage-gateway.ts`；`production-h3-graph.ts`），ComfyUI 必须能读该目录。`comfyBaseUrl` 明文 HTTP 仅 loopback，跨机需 HTTPS + bearer。

| 主机 | 组件 | 说明 |
|---|---|---|
| writing-loop-sg（10.148.0.5，常驻） | control plane workspace（ProductionStore 账本）、调度器、`production-worker` systemd timer、gateway 实例 A（jobs / stages / ingest kernel；Ark、Vertex 的 provider adapter）、TLS 终结（caddy 或 nginx 反向代理，只绑定私网 IP，不复用现有 0.0.0.0/0 的 443 防火墙规则） | 账本只在服务器 workspace 写入；本机 `~/dramas` 为 paused 副本，不并行写入。plan-shots / confirm / qc / handoff 经 ssh 在服务器执行 |
| GPU VM（Spot g4-standard-48，镜像 `wl-comfy-h3-g4-sg`，按批次启停） | ComfyUI、gateway 实例 B（jobs + stages kernel，H3 profile；objects 目录与 ComfyUI 同一文件系统）、TLS 代理（实例 B 的 jobs / stages 端点与 ComfyUI `/view`，bearer） | 仅 H3 批次期间运行；jobs kernel 的 job record 持久化在 VM 持久盘，抢占后重启可对账（§7） |
| 本机 | VCS（scripted-drama 流水线）、Blender 候选图轨道 | handoff 导出目录由服务器 rsync 到本机；本机 worker 不直达私网 gateway |

worker runtime config 中 `backends[]`（每个 backendInstanceId 独立 baseUrl）与 `stagingProfiles[]`（每个 profile 独立 baseUrl）分别指向实例 A 或 B；`gateway`（ingest）单一 baseUrl 指向实例 A，实例 A 的 `comfyBaseUrl` 指向 GPU VM 上的 TLS 代理（H3 输出取回），云后端输出走 provider-output 路径。TLS 证书：私有 CA 签发，worker 通过 `NODE_EXTRA_CA_CERTS` 信任；或内网域名 DNS-01 证书，Phase 1 二选一。`credentialEnv` 由 systemd `EnvironmentFile`（0600）注入；runtime config 文件权限 0400/0600。

### 8.1 Phase 0：契约与编译器（零网络）

| 项 | 内容 |
|---|---|
| 改动文件 | 新增 `hub/src/production-shot-request.ts`、`hub/src/production-provider-adapter.ts`；修改 `hub/src/production-intent.ts`、`production-adapter.ts`、`production-domain.ts`、`production-coordinator.ts`、`production-coordinator-domain.ts`、`production-reconcile.ts`、`production-ingestor.ts`、`production-read-model.ts`、`production.ts`、`cli.ts`、`visual-production.ts`；`docs/design/phase-3-remote-production/AI-SPEC.md`；`references/config-schema.md`、`references/visual-production-schema.md`；`hub/test/production-*.ts` |
| 交付物 | ShotRequest 类型、解析、`deriveVideoMode`、`compileShotRequest`、`shotRequestFromScript`；intent 新枚举、两个云 execution 分支、三个 gate、license `obligations`；capability 与 locator 联合类型；`ProductionProviderAdapter` 接口、`PreparedProviderSubmission` 与 `ComfyUiAdapter` 包装；cost 扩展；`production plan-shots`、`production qc`、`visual approve-candidate` 子命令；`visual/production.v1.json` 的 `shotIds`、`subjectReferences[]` 与 `mappings.v1.json`、`prop-states.v1.json` schema；AI-SPEC fixture 更新与「H3 输出不得用于改进其他模型」使用约束 |
| 验收标准 | 现有 `hub/test/production-*.ts` 全部通过；旧 H3 intent JSON 解析结果与 idempotencyKey 不变；validate 矩阵测试（每家后端 × 每个错误码至少 1 例，含 `prompt_contains_provider_directive`、`prompt_language_unsupported`、`dialogue_language_unsupported`、`reference_index_out_of_range`、`license_obligation_unmet`）；seedance 分支拒绝 `seed ≠ null`、fast + 1080p、首尾帧与参考混用、2.0 仅音频参考；veo 分支拒绝 6 s ref2v、lite + ref2v、lite + 4k、fast + 4k（探针前）、1080p + 4 s、中文 prompt 无译文；veo 分支对每个 ShotRequest 记录 `seed-not-reproducible`；同一 draft 编译产生字节相同的 ShotRequest 与 intent draft；`--plan` 零写入（目录快照前后一致）；`ep-001.md` 1-1 场经 `script-lint.ts` 预填的 draft 在人工补齐 `camera` 字段后通过解析与编译（t2v 模式）；Ark cancel 状态机测试含「DELETE 前状态已变为 succeeded」 |
| 前提条件（并行外部，负责人：项目操作者；截止：Phase 0 验收前；未完成则触发 Phase 1 后端裁定） | (a) BytePlus 账户注册与认证、充值 ≥30 USD、Seedance 模型开通、签发 `BYTEPLUS_ARK_API_KEY`；(b) 在 writing-loop-sg 上核实 ffmpeg / ffprobe、Node 版本、systemd 权限（本轮 ssh 失败未能核实）；(c) H3 live `/prompt` 探针（零新代码：`ssh -L` + curl 或 VCS `tools/_comfyui/client.py` 的 `/system_stats`、`/object_info`、`/prompt`），在 `wl-comfy-h3-g4-sg` 镜像上验证 cuda132 驱动 + int8_convrot + ComfyUI 0.31 节点 schema 与契约一致，结果写入 h3-comfy-deployment-status；(d) S01 候选图批准轨道启动（§6.2） |

### 8.2 Phase 1：gateway 进程、部署与第一家云后端

| 项 | 内容 |
|---|---|
| 改动文件 | 新增 `hub/src/production-gateway-runtime-config.ts`、`production-gateway-main.ts`、`production-ark-adapter.ts`；修改 `production-job-gateway.ts`、`production-stage-gateway.ts`、`production-gateway.ts`、`production-gateway-router.ts`、`production-runtime-config.ts`；部署脚本（systemd 单元、TLS 代理配置、rsync 脚本）；测试 `hub/test/production-ark-adapter.ts` 与 parity fixture |
| 交付物 | server-owned registry（backends kind、凭据形态、execution profile、`allowedProcessingRegions`、reconcilePolicy、准入上限、汇率、cas authority 与 `ProductionStageAssetResolver`）与 gateway 进程装配；实例 A 部署到 writing-loop-sg，TLS 终结（caddy / nginx，私网 IP，私有 CA 或内网域名证书）、证书分发到 worker（`NODE_EXTRA_CA_CERTS`）、credentialEnv 经 systemd `EnvironmentFile`（0600）发放、`production-worker` systemd timer（≤6 h）、`handoff --export-dir` 输出 rsync 到本机的脚本；Ark adapter（endpoint host 参数化：火山 / BytePlus）；stage kernel `slotPolicy`、ShotRequest 内容校验与 `cas://` resolver；jobs kernel `providerJobId` 持久化、profile 双形态与 `PreparedProviderSubmission`；ingest kernel `provider-output` 路径；从 `tests/tools/test_seedance_volcengine.py` 导出请求 payload 与状态机 parity fixture（范围限定为 payload 形状与状态机；该测试的定价基于火山 CNY 表），BytePlus USD 价目以官方计价页单独建 fixture |
| 后端裁定（Phase 0 验收时） | BytePlus 账户就绪 → Phase 1 后端为 Seedance over BytePlus（默认）；未就绪 → Phase 1 后端改为 Veo fast（凭据 veo-m2m.json 与 us-central1 已验证，inline 模式无 bucket），Seedance 顺延到 Phase 3，Veo 的 Phase 3 交付项前移。两种情况下 gateway / TLS / 部署交付相同 |
| 验收标准 | worker 经 HTTPS 提交到 gateway，credential 错误被拒、正确被受理各 1 例；录制 Ark 响应 fixture 覆盖 queued / running / succeeded / failed（含 `InvalidParameter.*`）/ expired / NotFound 与 429，以及「body 参数与文本指令冲突」实测以确定优先级；一条真实 t2v 任务 4 s / 480p / 9:16（`doubao-seedance-2-0-260128` 等价 BytePlus modelId，官方示例 480p 0.07 USD/s → 约 0.28 USD；若用 2.5 约 0.41 USD）经 `plan → confirm → worker --once → qc --approve → handoff` 全链路，产出主视频与尾帧两个 AssetRef，cost basis `reported`，reservation 释放；第二条 i2v 任务以第一条尾帧作 `firstFrame`（承接链，经 `cas://` 再登记）入库；两条任务直接为 9:16 并保留为 Phase 2 输入；Veo 替代路径的验收为一条 fast 4 s / 720p / 9:16（约 0.40 USD） |
| 前提条件 | Phase 0 验收通过；§8.0 的主机、TLS 方案（用户决定）、账户裁定 |

### 8.3 Phase 2：VCS 导入与合成（EP001 场 1-1 端到端试点，只用 Phase 1 的后端）
...
| 验收标准 | 导入后 `studio.py status` 报告 `next_stage: proposal`，proposal 批准后报告 `next_stage: edit`；`video_compose` 以 ffmpeg runtime 渲染 9:16 成片，`_pre_compose_validation` 通过；Backlot `gate_skipped` 为空；importer 契约测试覆盖 digest 不匹配、sha256 不匹配、非 approved take 均拒绝；publish 署名检查对含 H3 take 的 manifest 生效（fixture） |
| 前提条件 | Phase 1 的 9:16 takes；1-1 共 4 个 ▲ 行（【特效】×2、【特写】×1、1 条 VO）的新批次约 4 × 5 s × 0.15 USD/s（Seedance 2.0 720p）≈ 3 USD；本机 VCS 环境（Python 环境、ffmpeg、`pipeline_defs` 新清单）；服务器 `handoff --export-dir` 后 rsync 到本机 |

### 8.4 Phase 3：Veo over Vertex（或 Seedance，按 Phase 1 裁定互换）

| 项 | 内容 |
|---|---|
| 改动文件 | 新增 `hub/src/production-vertex-veo-adapter.ts` 与测试；修改 `production-gateway.ts`（ffmpeg 尾帧提取）、`production-gateway-runtime-config.ts`（tariff 价目、`allowedProcessingRegions`）；writing-loop 写作流程增加英文译文产物 |
| 交付物 | Vertex adapter（service-account 文件凭据；v1 inline 模式：输入 bytesBase64Encoded ≤20 MB、输出 base64 → spool；`ioMode: gcs` 可选，附 bucket / IAM / 区域核实项）；ingest kernel ffmpeg 尾帧提取；`tariff` 计费；`processing-region-not-allowed` gate 接入项目配置（`allowedProcessingRegions` 加 US）；英文 prompt 译文的 draft 供给步骤（写作侧）；音频探针 |
...
| 前提条件 | Phase 1 的 gateway 与部署；项目 `allowedProcessingRegions` 含 US；译文供给步骤已定义（用户决定） |

### 8.5 Phase 4：H3 live 与契约 v2

| 项 | 内容 |
|---|---|
| 改动文件 | `hub/src/production-h3-graph.ts`（绑定契约与 `validateStageContracts` 增加 `shot-request` slot；参数投影对 sentinel 模板计算）、`production-runtime-config.ts`（按家族分派的 binding verifier）、`production-shot-request.ts`（H3 分支）、`production-job-gateway.ts`（实例 B 的 preempted 改写）；GPU VM 侧：Spot 创建脚本、实例 B systemd 服务单元、TLS 代理配置 |
| 交付物 | H3 graph 契约 v2：`generator.prompt` 与 `RandomNoise.noise_seed` 为 sentinel `writing-loop://shot-request/<profileId>/prompt\|seed`，参数投影对 sentinel 模板计算，materialize 时从 ShotRequest 填入并重新断言整图；stage 契约 v2 在固定绑定前增加 index 0 的 `shot-request` slot，LoadImage 绑定 index 顺延；`compileShotRequest` H3 分支（durationGrid 取已配置 profile 集合）；worker 侧按家族分派的 binding verifier；H3 输出经同一尾帧提取路径；实例 B 重启后的 `provider_failed:preempted` 改写；每个 H3 批次绑定人工批准的 VM 时间窗 |
| 验收标准 | `assertGraph` 对 v1 模板结果不变；同一 ShotRequest 分别编译为三家 intent 时 `inputs` 的 AssetRef 集合相同（inputs[0] 除外）；H3 fl2va 8 s 9:16 一条任务入库并 handoff；SaveVideo 在 `/history` 中的分组名以实测写入 `outputLocators` 测试；模拟抢占（停止 VM）后重启，任务被改写为 `provider_failed:preempted` 且 reservation 处理正确 |
| 前提条件 | Spot G4 实例创建脚本与抢占后手动重启流程（按需 RTX PRO 6000 配额为 0，无按需回退；Spot 配额 8）；开启服务与地域门；从镜像导出权重 sha256 清单写入 runtime config modelBundle attestation（`h3GraphContract` 需每个组件的 `artifactSha256`；镜像构建时的清单存放位置未记录）；确定 useTerritories / deploymentTerritories，若含 US / EU / GB / KR 先在 platform.minimax.io/h3-license 申请并把 evidence / licenseSha256 写入 intent license；项目 `licenseCompliance` 配置（年收入声明、署名面）；实例 B 随镜像启动的 systemd 服务单元、TLS 代理与 bearer；Phase 0 的 live `/prompt` 探针结论；GCP 月度余量决策（§4.7） |
...
| 文件 | 改动 |
|---|---|
| `production-intent.ts` | 枚举扩展；两个云 execution 分支；`parseProductionIntentExecution` 新增解析（静态字段与 modelId 级约束）；`parseInputs` 增加 `inputs[0]` mediaType 必须为 shot-request 的检查（云家族；H3 契约 v2 后同样适用）；license evidence `obligations`；gate code、gate context、deny 逻辑 |
| `production-shot-request.ts`（新增） | 类型、解析、编译、`shotRequestFromScript` |
...
| `production-h3-graph.ts`（Phase 4） | 绑定契约与 `validateStageContracts` 增加 `shot-request` slot；参数投影对 sentinel 模板计算 |
...
1. Veo 音频能力文档冲突只在 generate-001 与 fast 上（模型页 Not supported、计价页 Video + Audio）；lite 探针不能推及其他 modelId。GA / fast 若无音频，含对白镜头改走 Seedance 或 H3；Phase 3 按 modelId 探针决定 profile 值。
2. Veo：fast 4k 文档冲突（模型页无、计价页有）、1080p / 4k 强制 8 s（Gemini API 文档明文，Vertex 操作指南无说明，本文采用保守规则）、personGeneration 接受值（三份文档互不一致，v1 不下发）、并发上限均未核实；`global` 区域未核实，本文不支持。LRO 结果保留期未核实（inline spool 规避）。gcs 模式的 bucket、IAM、区域三项未核实。
3. Ark / Vertex 无幂等键；`submission-unknown` 对账依赖 `safety_identifier` 作为相关键，该用法是否符合平台使用政策未核实；未确认前 reconcilePolicy 为 `none`，最坏情况为 `execution_expires_after` 内排队孤儿产生费用并计入 exposure。
4. Ark 输出 URL 24 h 有效期与 worker 调度间隔耦合；停摆超过 24 h 导致已计费任务 `output_expired`。
5. 火山 CNY 计费需汇率配置；默认先接 BytePlus（USD）回避。
6. Seedance 2.x 拒绝真人人脸参考且 seed 不可用；跨镜头人物一致性依赖参考图与首尾帧链。2.x 对 seed 参数忽略还是报错未核实；body 参数与文本指令冲突时的优先级未核实（Phase 1 fixture 实测）；查询接口对已过期 / 已删除任务返回的 HTTP 状态码未核实。
7. H3 graph 未经 live 验证（Phase 0 并行探针）；契约 v2 改变参数投影范围，需与 ComfyUI ≥0.30.0 的 MiniMaxH3 节点契约实测对齐；SaveVideo 输出分组名未核实；权重 sha256 清单存放位置未记录；按需 RTX PRO 6000 配额为 0，Spot 抢占即任务丢失。
8. 分镜数据缺口（景别、机位、时长、站位、approved 候选图为空）：EP001 试点前需补齐 `prop-states.v1.json`、`mappings.v1.json`；候选图批准为并行轨道，v1 关键帧来源不含 approved-candidate。
9. 镜头粒度：全季约 1910 个 ▲ 行，每集 60 秒预算需要合并规则（`stageGroup` 内合并），合并依据未定义。
10. 输出语言与口型：中文母本与出海英文 profile 并存，`dialogue[].language` 需项目级裁定；Veo 仅接受英文 prompt，译文由写作侧提供，含中文口型对白的镜头不能用 Veo。
11. gateway 进程装配当前不在仓库内，Phase 1 之前云后端改动只能以 fixture 验证；Phase 1 新增的部署前提（主机、TLS 终结、证书分发、credential 发放、timer）在完成前无法进行任何真实任务；writing-loop-sg 上 ffmpeg / ffprobe、Node 版本、systemd 权限、是否已有反向代理未核实（本轮 ssh 失败）。
12. VCS 预授权检查点依赖 GateRecord 与 decision_log 解释来源；`get_next_stage` 在 proposal 未完成而 scene_plan / assets 已完成时的表现以 Phase 2 验收为准。
13. `MAX_PRODUCTION_INTENT_INPUTS = 32` 与 Seedance 2.5 的 30 张参考上限接近；ShotRequest 的 12 张限制为设计约束。
14. BytePlus 账户、密钥、余额当前不存在（本机无 `.env`，环境无 ARK / BYTEPLUS 变量）；Phase 1 后端在 Phase 0 验收时裁定，未就绪则 Veo 先行。
15. GCP 项目 €50/月硬顶与 `stop_billing` 覆盖调度器、GPU VM 与 Veo；项目现有常驻资源目录价已超顶，抵扣金状态未核实；EP001 全量 Veo 费用单独即超顶。bulk 前必须做出提高硬顶或迁移项目的决策（§4.7）。
16. writing-loop-sg 的 VM 默认 SA scope 不含 cloud-platform，`google-adc` 在该主机不可用；v1 用 service-account 文件，换绑 SA 需停机。
17. MiniMax H3 Community License 的年收入阈值与署名义务需项目级声明（`licenseCompliance`）；若含 US / EU / GB / KR 部署需先取得书面许可。
18. Veo 英文 prompt 译文的供给步骤（写作侧 agent 或人工）尚未定义，Phase 3 前需在 writing-loop 写作流程中增加该产物。
...
| 项 | 影响范围 | 核实方式 / 阶段 |
|---|---|---|
| Veo generate-001 / fast 原生音频 | `nativeAudio.status`、含对白镜头路由 | Phase 3 按 modelId 探针，记录 operation name |
| Veo fast 4k | `parseProductionIntentExecution` veo 分支 | Phase 3 实测写入 `profile.limits`，之前拒绝 |
| Veo 1080p / 4k 是否强制 8 s | 时长网格 `gridByResolution` | Phase 3 实测，之前固定 {8} |
| Veo personGeneration 接受值 | 不下发 | Phase 3 可选探针 |
...
| `safety_identifier` 用作相关键的平台政策 | Ark reconcilePolicy | 用户决定 / 向平台确认 |
| Seedance 2.x seed 参数行为、body 与文本指令冲突优先级、NotFound HTTP 状态码 | 编译规则、fixture | Phase 1 录制 fixture |
| Seedance 对白口型质量 | 无官方指标 | 样片评估 |
| H3 graph live 行为、SaveVideo 分组名、节点 schema 与契约一致性、prompt 语言限制 | Phase 4 契约 v2 | Phase 0 并行探针 + Phase 4 实测 |
| H3 权重 sha256 清单存放位置 | modelBundle attestation | 从镜像导出 |
| H3 视频延长能力 | 非目标 | 未核实 |
| writing-loop-sg 的 ffmpeg / ffprobe、Node、systemd 权限、反向代理 | Phase 1 部署 | Phase 0 并行前置 (b) |
| GCP 抵扣金状态 | 月度余量门取值 | 操作者核实 |

### 9.4 权衡

| 方案 | 收益 | 影响 |
|---|---|---|
| 云后端 kind 只在 gateway registry 扩展，worker 侧 backends[].kind 保持 `comfyui \| production-gateway` | 凭据不进入 worker | 没有云后端的开发期直连通道，联调必须先部署 gateway 进程，且 jobs 端点必须为 HTTPS + credential（loopback 不豁免），本机联调也需要自签证书或本地反代 |
| gateway 双实例（实例 A 常驻 writing-loop-sg 服务云后端与 ingest，实例 B 随 GPU VM 服务 H3） | stage kernel 的硬链接模型与 ComfyUI 同一文件系统的要求得以保持 | 两处 TLS 端点、跨实例 CAS 取回与 VM 抢占后的对账。替代方案为 stage kernel 增加远端上传模式（`/upload/image`），需放弃 CAS 硬链接模型，本文不采用 |
...
| Veo 只接受英文 prompt，编译器只选用 draft 中已提供的译文并记 `prompt-translated` 强制重审批 | worker / gateway 不调用 LLM 的约束得以保持 | 写作侧多一个译文产物，含中文口型对白的镜头不能用 Veo |
...
| 新增 tariff 与 reported-converted basis | Veo、H3、火山任务可释放预留 | 账面金额可信度低于 reported / billed，读模型需分列；GPU VM 小时不在 tariff 内，改由月度余量门覆盖 |
...
| Phase 1 后端在 Phase 0 验收时裁定（BytePlus 就绪则 Seedance，否则 Veo fast 先行） | 第一条真实任务不被外部账户开通阻塞 | Phase 1 / 3 的交付项可能互换，Veo 先行时首个试点镜头受英文 prompt 与音频未核实的限制 |

### 9.5 需决策事项

| 事项 | 选项 | 截止点 |
|---|---|---|
| Phase 1 后端 | Seedance over BytePlus（账户就绪）/ Veo fast 先行 | Phase 0 验收 |
| TLS 证书方案 | 私有 CA + `NODE_EXTRA_CA_CERTS` / 内网域名 DNS-01 | Phase 1 部署前 |
| GCP €50/月硬顶 | 提高硬顶 / 迁移到独立 GCP 项目与预算 | 任一 `phase: bulk` 前 |
| `availableBudgetMicros` 首次取值与维护 | 按「€50 − 当月固定支出」设置，记录 `maintainedBy / asOf` | Phase 1 前 |
| 输出语言与口型 | `dialogue[].language` 为中文或英文；是否需要口型对齐 | Phase 0 试点 draft 前 |
| 镜头合并规则 | `stageGroup` 内合并依据 | EP001 全集批次前 |
| Veo 译文供给步骤 | 写作侧 agent 步骤 / 人工 | Phase 3 前 |
| MiniMax H3 许可 | useTerritories / deploymentTerritories；年收入声明；署名面；是否申请书面许可 | Phase 4 前置 |
| `safety_identifier` 对账用法 | 确认符合平台政策后启用 `list-window`，否则保持 `none` | Phase 1 fixture 前 |
